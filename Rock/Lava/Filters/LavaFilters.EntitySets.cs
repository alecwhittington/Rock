...
namespace Rock.Lava
{
    internal static partial class LavaFilters
    {
        ...
        public static EntitySet CreateEntitySet( ILavaRenderContext context, object input, string entityType = null, string expireInMinutes = null, string entitySetPurposeValueId = null, string note = null, string parentEntitySetId = null )
        {
            ...
            if ( entityType != null )
            {
                entityTypeId = EntityTypeCache.Get( entityType.ToString(), allowIntegerIdentifier: true )?.Id;

                if ( entityTypeId == null )
                {
                    // Try to find the EntityType by name.
                    entityTypeId = EntityTypeCache.GetId( entityType.ToString() );
                }
            }

            if ( entityTypeId == null )
            {
                throw new Exception( $"CreateEntitySet failed. Entity Type is invalid. [EntityType={entityType}]" );
            }
            ...
            else if ( input is string inputString )
            {
                ...
                if ( !isValid )
                {
                    throw new Exception( $"CreateEntitySet failed. Entity identifier list is invalid. [Input={inputString.Truncate(100)}]" );
                }
            }
